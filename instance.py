...
parser = argparse.ArgumentParser(description='Usage for this file in explained below')
#port = sys.argv[1]
...
def Main(port, activeConnections,instance):
   
    options = {"model": instance+"/yolo.cfg", "load": "bin/yolo.weights", "threshold": 0.6,"gpu":0.1,"labels":"labels.txt"}
    tfnet = TFNet(options)
    ...
    threads = []
    try:
        print("Instance are ready to serve....")
...
if __name__ == '__main__':
 parser.add_argument('pos_startPort', type=int,help='Starting Port of the Instance')
 parser.add_argument('pos_numberOfInstance', type=int,help='Number of Instiantiations')
 parser.add_argument('pos_config',help='Folder name where configuration  yolo configuration file is stored(must be inside same directory where the script is executed)')  
 args = parser.parse_args()
 Main(int(args.pos_startPort), int(args.pos_numberOfInstance),args.pos_config)
